...
import { languageLabels } from '@/frontend/features/translation';
...
export const CreateProductForm = () => {
  ...
  const { register, control, fields, errors, onSubmit, isPending, isSuccess, error } = useCreateProductForm();
    ...
    <form onSubmit={onSubmit} className="flex flex-col gap-4 w-full max-w-md">
      ...
      <Tabs defaultValue={fields[0]?.language}>
        <TabsList className="w-full">
          {fields.map((field) => (
            <TabsTrigger key={field.id} value={field.language} className="flex-1">
              {languageLabels[field.language]}
            </TabsTrigger>
          ))}
        </TabsList>

        {fields.map((field, index) => (
          <TranslationTabContent key={field.id} lng={field.language} index={index} errors={errors} register={register} />
        ))}
      </Tabs>
