...
- **`AuthStatus` dropdown** — `<DropdownMenuItem asChild>` wraps `<SignOutButton>` so Radix's close handler and the button's own `onClick` both fire without nesting `<button>` inside `<button>`. It also contains a **"My Orders"** item — a locale-aware `<Link href={ROUTES.ORDERS}>` (Package icon + `myOrders.title` label) — which is the desktop/web entry point to the customer `my-orders` page (the mobile burger uses `NavMobileMenu`'s own Orders link instead). Because the dropdown only renders when `session?.user` is truthy, the item is inherently signed-in only.
